...
export default function GameplayOverview({ project }: { project: any }) {
  ...
  const cards: { title: string; description: string; image?: string }[] = project.gameplay?.length
    ? project.gameplay
  ...
  // Shared fallback poster for projects that don't supply per-card images
  const fallbackPoster = project.media?.screenshot ?? project.media?.banner;
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 sm:px-6">
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        {cards.map((card, i) => {
          const thumb = card.image ?? fallbackPoster;
          return (
