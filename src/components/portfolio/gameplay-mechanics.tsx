...
function MechanicCard({ mechanic: m }: { mechanic: GameplayMechanic }) {
  ...
  const hoverVideo = m.media?.hoverVideo;
  const previewImg = m.media?.preview ?? m.media?.gif;
  const previewVideo = m.media?.video;
  const hasLinks = m.links?.github || m.links?.demo;
  const showHoverMedia = isHovering && Boolean(hoverVideo || m.media?.gif);
  const hasExtras =
    m.difficulty || m.engineVersion || m.category || m.docsUrl || m.articleUrl || m.sourceUrl;

  return (
    <article
      className="card-lift cover-zoom group relative flex h-[320px] w-[280px] shrink-0 snap-start flex-col overflow-hidden rounded-sm border border-white/8 bg-[#2B2E35] sm:w-[320px]"
