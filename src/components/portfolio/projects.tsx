...
import { Play, ExternalLink, ArrowRight, Filter, Rocket, Linkedin } from "lucide-react";
...
export function Projects() {
  ...
  const unityGrouped = useMemo(
    () => all.filter((p) => p.tags.includes("unity")),
    [all],
  );
...
function ProjectCard({ project }: { project: Project }) {
  ...
  const hoverVideo = project.media?.hoverVideo;
  const showHoverMedia = isHovering && Boolean(hoverVideo || project.media?.gif);
  const linkedInHref =
  ...
  return (
    ...
    >
      ...
      <Link
        ...
        className="cover-zoom relative block aspect-[460/215] overflow-hidden border-b border-white/5"
      >
          ...
          <>
            <img
              ...
              className={`absolute inset-0 h-full w-full object-cover transition-opacity duration-300 ${
                showHoverMedia ? "opacity-0" : "opacity-100"
              }`}
              onError={(e) => {
